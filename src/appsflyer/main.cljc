(ns appsflyer.main
  "Kotodama WASM entrypoint for the Appsflyer clean-room actor (L4) — Clojure port.
  ...
  facts (`appsflyer.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "appsflyer")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"appsflyer.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "appsflyer-compat" :tier tier :entities entities} 200])
...
(defn start! [] :appsflyer-compat/ready)
